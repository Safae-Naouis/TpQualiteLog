...
public class BanqueDaoImpl implements IBanqueDao {
	@PersistenceContext // on a besoin de cette annotation --> parceque apres on aura besoin de
						// configurer JPA avec Spring
// Pour gerer la persistance , on doit faire appel e JPA --> d'oe l'attribut 'EntityManager' qui permet de faire le mapping objet relationnel ORM
	private EntityManager em; // ce attribut pour le mapping objet relationnel ORM

	// on va utiliser Hibernate mais, On fait au debut appel e JPA et apres on
	// decide qu'est ce qu'on va utiliser ( (JPA/Hibernate) ou (JPA/TopLink) ) -->
	// on gere les transations avec Spring
	@Override
	public Client addClient(Client c) {
		em.persist(c); // code tres simple (puisqu'on gere les transactions avec Spring) --> ensuite ,
						// je retourne le client enregistre
		return c;
	}
	...
	public Employe addEmploye(Employe e, Long codeSup) {
		...
		em.persist(e); // l'employe 'e' est l'objet Employe apres avoir associer e son attribut
						// 'employeSuperieur' l'objet Employe touver e partir du codeSup (parametre non
						// nulle de la function)
		return e;
	}
	...
	@Override
	public void addEmployeToGroupe(Long codeEmp, Long codeGr) { // on doit chercher les deux objets Employe et Groupe
																// --> puis les associer

		Employe employe = em.find(Employe.class, codeEmp);
		Groupe groupe = em.find(Groupe.class, codeGr);

		// puisque l'association (entre les entity) est dans les deux sens
		// (bidirectionnel) --> donc , on doit ajouter dans les deux sens
	...
	public Compte getCompte(String codeCompte) {
		...
		// on doit gerer le cas oe on n'a pas trouver un compte --> lever l'exception et
		// afficher un message
		if (compte == null)
	...
	public List<Operation> getOperationsCompte(String codeCompte) {
		// on va utiliser cette fois ci 'Query' ---- ( on doit choisir Query de JPA eed
		// javax.persistence.Query)
		// 'find' , on l'utilise quand on va chercher un objet sachant sa cle primere
		// --> si non (si c'est une liste), on doit passer par une requete 'Query'
		Query request = em.createQuery("select op from Operation op where op.compte.codeCompte =: x "); // CODE_COMPTE
																										// est le non de
																										// la colonne
																										// (de
																										// l'attribut
		...
		// retourner le resultat trouve
		return request.getResultList();
	}

	@Override
	public Client getClient(Long codeClient) {
		// recherche de client e partir de son cle primere --> donc , on utilise 'find'
		// et non pas une requete
		Client client = em.find(Client.class, codeClient);

		// on doit gerer le cas oe on n'a pas trouver un client --> lever l'exception et
		// afficher un message
		if (client == null)
